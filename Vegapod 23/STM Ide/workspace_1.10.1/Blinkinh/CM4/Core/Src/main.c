...
uint32_t sinFreq = 50,
		maxCount, samples = 51;
//uint32_t *lookUp1, *lookUp2, *lookUp3;
uint32_t lookUp1[] = {
  800.00,
  1047.21,
  1270.23,
...
//	      t = 0;
//	    }
//	    // Generating sin value
//	    sinv = sin(2 * 180 * fs * ts) * 256;
//	    sinu = sin((2 * 180 * fs * ts) + 360) * 256;
//	    sinw = sin((2 * 180 * fs * ts) + 720) * 256;
//
//	    trig_val = ((t - T/2) * 900 * A)/(T);
//
//	    t = t + (micros() - lastt);
//	    ts = ts + ((double)(micros() - lastt) / 100000000.0);
//	    lastt = micros();
//
//
//	    if (sinv >= trig_val)
